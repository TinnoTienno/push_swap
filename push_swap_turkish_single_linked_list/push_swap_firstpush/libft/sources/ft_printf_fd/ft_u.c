...
/*   Updated: 2024/02/05 13:50:40 by eschussl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

int	ft_zeru(int fd, t_printfstruct structure)
{
	int	i;
	int	count;
...
int	ft_u(int fd, t_printfstruct structure, unsigned int nb)
{
	...
	if (!str)
		return (0);
	count = ft_putlstr_fd(fd, str, ft_strlen(str));
	free (str);
	return (count);
}
